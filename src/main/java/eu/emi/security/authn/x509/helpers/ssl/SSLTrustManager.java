...
public class SSLTrustManager implements X509TrustManager
{
	...
	protected void checkIfTrusted(X509Certificate[] certChain) throws CertificateException
	{
		...
		if (!result.isValid())
			throw validationException(certChain, result);
	}

	protected static CertificateException validationException(
			X509Certificate[] certChain, ValidationResult result)
	{
		String subject = "";
		if (certChain != null && certChain.length > 0 && certChain[0] != null)
			subject = certChain[0].getSubjectX500Principal().getName();
		String message = "The peer's certificate with subject's DN " + subject +
				" was rejected. The peer's certificate status is: " + result;
		ValidationError primaryError = result.getPrimaryError();
		Throwable cause = primaryError == null ? null : primaryError.getCause();
		return cause == null ? new CertificateException(message) :
